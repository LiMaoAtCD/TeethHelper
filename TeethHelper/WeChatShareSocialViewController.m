//
//  WeChatShareSocialViewController.m
//  TeethHelper
//
...
@interface WeChatShareSocialViewController ()
@property (strong, nonatomic) UIView *bgView;


@end

@implementation WeChatShareSocialViewController
...
- (void)viewDidLoad {
    ...
    [friends mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerX.equalTo(shareView.mas_centerX).multipliedBy(0.5).offset(0);
    ...
    [socialButton setImage:[UIImage imageNamed:@"icon_shequ_share"] forState:UIControlStateNormal];
    [socialButton addTarget:self action:@selector(SharetoSocial:) forControlEvents:UIControlEventTouchUpInside];
    [shareView addSubview:socialButton];
    
    [socialButton mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerX.equalTo(shareView.mas_centerX).multipliedBy(1.5);
        make.height.equalTo(@50);
        make.centerY.equalTo(shareView.mas_centerY).offset(-30);
        make.width.equalTo(@50);
    }];
    UILabel *socialLabel = [[UILabel alloc] initWithFrame:CGRectZero];
    
    socialLabel.text = @"社区";
    socialLabel.textColor = [UIColor blackColor];
    socialLabel.textAlignment = NSTextAlignmentCenter;
    
    [shareView addSubview:socialLabel];
    
    [socialLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(socialButton.mas_bottom).offset(0);
        make.height.equalTo(@21);
        make.centerX.equalTo(socialButton.mas_centerX);
...
-(void)SharetoSocial:(id)sender{
    ...
    if ([self.delegate respondsToSelector:@selector(didShareToSocialClicked)]) {
        [self.delegate didShareToSocialClicked];
    }
    [self dismissViewControllerAnimated:YES completion:nil];
